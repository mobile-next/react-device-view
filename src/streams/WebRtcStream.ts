...
export class WebRtcStream {
  ...
  private async logSessionIdentity(): Promise<void> {
    try {
      ...
      console.log(`device-view: session correlation sessionId=${this.session.sessionId} ssrc=${ssrc ?? 'unknown'} utc=${new Date().toISOString()}`);
    } catch (err) {
  ...
  private async logReceiverStats(): Promise<void> {
    try {
      ...
      const jbDelayMs = dEmitted > 0 ? Math.round((((v.jitterBufferDelay ?? 0) - (prev.jitterBufferDelay ?? 0)) / dEmitted) * 1000) : 0;
      const jbTargetMs = dEmitted > 0 ? Math.round((((v.jitterBufferTargetDelay ?? 0) - (prev.jitterBufferTargetDelay ?? 0)) / dEmitted) * 1000) : 0;
  ...
  private async sendOfferToWebrtcServerWithRetry(
    url: string,
    sessionId: string,
